function SocksV5(addr, port, streaming){
    ...
    function socketSendSocksV5Connection(destAddr, destPort){
        ...
        if('01' == addrType){
            ...
            addrRepr = $.nodejs.buffer.Buffer.concat([
                new $.nodejs.buffer.Buffer([destAddr.length,]),
                new $.nodejs.buffer.Buffer(destAddr, 'ascii'),
            ]);
        };
    ...
    socket.on('data', function(data){
        ...
        if(3 == state){
            if(5 == data.readUInt8(0) && 0 == data.readUInt8(1)){
                console.log('Good, ready to send!');
                state = 4;
    ...
    this.connect = function(parameters){
        ...
        socket.connect(port, addr, function(){
            ...
            socket.write(new $.nodejs.buffer.Buffer('050100', 'hex'));
            state = 1;
        });
    };
    ...
    this.write = function(data){
        if(state < 4) return;
        console.log(state, 'received such data: ', data);
        socket.write(data);
    };
